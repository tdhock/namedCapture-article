...
column. For example the pattern below uses \verb|name| for the first group:
...
This feature makes it easy to select particular elements of the extracted data by name.
...
pos.pattern <- list("[0-9,]+", int.from.digits)
namedCapture::str_match_named(
  subject.vec, 
  chrom="chr.*?",
  ":",
...
range.pattern <- list(
  "[[]",
  task1=int.from.digits,
  list(
    "-",
    taskN=int.from.digits,
  ), "?",
  "[]]")
task.pattern <- list(
  task=int.from.digits,
...
(task.df <- namedCapture::df_match_variable(
  ...
  Elapsed=list(
    hours=int.from.digits,
    ":",
    minutes=int.from.digits,
    ":",
    seconds=int.from.digits)))
