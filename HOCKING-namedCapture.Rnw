...
\abstract{Regular expressions are powerful tools for manipulating
  ...
  (\CRANpkg{rex}, \CRANpkg{stringr}, \CRANpkg{stringi}, \CRANpkg{tidyr}, \CRANpkg{rematch2}, \CRANpkg{re2r}).}
...
pattern \code{(?<start>[0-9]+)-(?<end>[0-9]+)} further provides access
...
\hline
 \pkg{base}         & \code{regexpr}                  & \code{gregexpr}                & PCRE/TRE \\
  ...
  \CRANpkg{namedCapture} & \code{str\_match\_*}, \code{df\_match\_variable}          & str\_match\_all\_*     & PCRE\\
...
expressions. It uses the PCRE library via the base \code{regexpr} and
\code{gregexpr} functions.
...
\subsection{Variable argument syntax: \code{*\_variable} functions}

In this section we introduce the variable argument syntax used in the
\code{*\_variable} functions, which is motivated by the desire to avoid
...
\code{str\_match\_variable} directly,
...
package with other R packages for regular expressions.
...
The text processing challenge is to extract the time and filetype for
...
Overall both rex and namedCapture provide good support for defining
regular expresions using a verbose, readable, and thus user-friendly
syntax. However there are two major differences:
\begin{itemize}
\item namedCapture assumes the user knows regular expressions and can
  write them as R string literals; rex assumes the user knows its functions,
  which generate regex strings. For the capture group time,
  \verb|none_of("]") %>% zero_or_more()| in rex gets translated to the
  regex string \verb|[^]]*|. Thus rex code is a bit more verbose than
  namedCapture.
\item In namedCapture type conversion functions are specified on the
  same line as the capture group name/pattern, whereas in rex conversions are
...
Below I show comparisons between \code{tidyr::extract} and
...
\item The namedCapture code is shorter. The tidyr code is longer
  ...
  inside the definition of \code{df\_match\_variable}.
