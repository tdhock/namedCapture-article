...
\item \CRANpkg{namedCapture} assumes the user knows regular
  ...
  strings. For the capture group time,
...
<<allPCRE, echo=FALSE, fig=TRUE, height=3, width=5>>=

library(data.table)
library(ggplot2)
timing.dt.list <- list()
titles <- c(
  log="First match, character vector",
  sacct="First match, two columns of a data.frame",
  trackDb="All matches in multi-line text file")
pkg.colors <- c(
...
for(task in c("log", "sacct", "trackDb")){
  ...
  if(file.exists(timing.rds)){
    timing.dt.list[[task]] <- data.table(
      task, title=titles[[task]], readRDS(timing.rds))
  }
}
...
gg.legend <- ggplot()+
  ...
  scale_x_log10(
    ...
    limits=c(10, 1e7))+
  geom_ribbon(aes(
    subject.size, ymin=q25, ymax=q75, fill=expr.chr.pkg),
    data=stats.dt,
    alpha=0.5)+
  geom_line(aes(
    subject.size, median, color=expr.chr.pkg),
    data=stats.dt)
gg.dl <- directlabels::direct.label(gg.legend, "last.polygons")
print(gg.dl)

@ 


<<pathological, echo=FALSE, fig=TRUE, height=3>>=

pathological <- readRDS("pathological.rds")
log.legend <- ggplot()+
  theme_bw()+
  theme(panel.margin=grid::unit(0, "lines"))+
