...
the TRE C library. 
Although TRE and PCRE offer similar features and performance for the vast majority of real-world
subjects and patterns, 
some malicious patterns can make PCRE run
quite a bit slower than TRE (see Section~``\sectiontimings'').
