...
to support ICU as an engine in \CRANpkg{namedCapture}. It is not possible,
because \CRANpkg{namedCapture} requires the engine to support output of
capture group names,
...
that allow the user to specify capture group names as R arguments
(as in the proposed variable argument syntax). Internally \CRANpkg{nc}
generates an un-named capture group for each named R argument,
and uses the specified argument name for the corresponding column name.
\CRANpkg{nc} is therefore able to provide a standard interface to
PCRE, RE2, and ICU.
