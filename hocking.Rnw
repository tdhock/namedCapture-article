...
group names in the regular expression pattern, and then extracting
...
All functions output a tibble (a data.frame subclass) with one row
...
in backreferences. However, the ICU library does not report group
...
\CRANpkg{rematch2} & chr/df     & chr          & tibble      & yes   & no    \\
