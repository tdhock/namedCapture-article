...
JobID column, along with the Elapsed column. Each column can be
processed in parallel by declaring a future plan:
...
complexity for both \CRANpkg{tidyr} and \CRANpkg{namedCapture} (using
either PCRE or RE2).
