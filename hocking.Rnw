...
\item Because \code{tidyr::extract} uses the ICU C library, which does
  ...
  \pkg{rematch2} supports specifying capture group names in regex
  string literals; \pkg{namedCapture} variable argument syntax
...
\item Since \code{rematch2::bind\_re\_match} returns character
  ...
  group-specific type conversion functions, which may be specified on the
...
\pkg{namedCapture} package with other R packages. For all of the
...
\pkg{re2r}, \pkg{stringr}, \pkg{stringi},
\pkg{rematch2}, \pkg{rex}. We expected small differences
...
\pkg{rex} and \pkg{rematch2} packages have significantly larger slopes than
the other packages
(\pkg{namedCapture}, \pkg{stringr}, \pkg{stringi}, and \pkg{re2r}). This can be explained because \pkg{rex} and \pkg{rematch2} use the base \code{gregexpr} and \code{substring} functions,
...
shows a comparison with \pkg{tidyr} and \pkg{rematch2}. Again we expected small
differences between the packages, and we observed linear time
complexity for \pkg{tidyr}, \pkg{rematch2}, and \pkg{namedCapture} (using
either PCRE or RE2).
...
guaranteed fast matching, RE2 must be used, via the \pkg{re2r} or
\pkg{namedCapture} packages.

\section{Discussion and conclusions}

Our comparisons showed how similar operations can be performed by
\pkg{namedCapture} and other R packages (e.g.\ \pkg{tidyr} and
\pkg{rex}). Our empirical timings revealed an inefficient
...
The article presented the \pkg{namedCapture} package, along with
detailed comparisons with other R packages for regular expressions.  A
unique feature of the \pkg{namedCapture} package is its compact and
...
the \pkg{namedCapture} package, which include support for 
...
(Table~\ref{tab:regex-libraries}), so one may wonder if it is possible
to support ICU as an engine in \pkg{namedCapture}. It is not possible,
because \pkg{namedCapture} requires the engine to support output of
capture group names,
which is not supported by ICU (Table~\ref{tab:regex-libraries}).
However ICU does support un-named capture groups,
so we have implemented support for the ICU engine
in a new package, \CRANpkg{nc}, which provides functions
that allow the user to specify output column names as R arguments
(as in the proposed variable argument syntax). Internally \pkg{nc}
generates an un-named capture group for each named R argument,
and uses the specified argument name for the corresponding output column name.
Therefore \pkg{nc} is able to provide a standard interface to three C regex libraries:
PCRE, RE2, and ICU.
...
pattern, then a function to specify the type conversion. Other
...
\pkg{namedCapture} package will be useful and inspiring for other
package developers.
