/* -*- compile-command: "make pcre_demo.out" -*- */
...
pcre *re;
const char *error;
char *pattern = "track (?P<trackName>[A-Za-z0-9_]+)";
char *subject;
...
if (argc - i != 1)
  {
  printf("One argument required: subject file name.\n");
  return 1;
  }
 slurp(argv[i], &subject, 0);
...
for (;;)
  ...
  //printf("subject_length=%d start_offset=%d\n", subject_length, start_offset);
  rc = pcre_exec(
    re,                   /* the compiled pattern */
    NULL,                 /* no extra data - we didn't study the pattern */
    subject,              /* the subject string */
